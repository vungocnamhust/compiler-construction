...
Token *readNumber(void)
{
  ...
  sprintf(str_max_int, "%d", INT_MAX);
  if (!(strlen(token->string) < strlen(str_max_int) || (strlen(token->string) == strlen(str_max_int) && strcmp(token->string, str_max_int) <= 0)))
  {
  ...
  if(check_float)
  {
    token->value1 = atof(token->string);
